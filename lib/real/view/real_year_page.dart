...
import '../model/rel_year_reg.dart';
import 'real_page.dart';

class RealYearPage extends StatefulWidget {
  const RealYearPage({
    super.key,
  });

  @override
  State<RealYearPage> createState() => _RealYearPageState();
}

class _RealYearPageState extends State<RealYearPage> {
  String filter = '';
  int endList = 70;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        ...
        List<ToCelda> titulos = state.realList?.celdasYear ?? [];
        List<RealYearReg> valores = state.realList?.listYear ?? [];
        valores = valores
            ...
            .toList();
        List<RealYearReg> valoresToShow = [];
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text("Real (Millones de pesos)"),
            actions: [
              const Gap(5),
              ElevatedButton(
                onPressed: () {
                      ...
                      itemBuilder: (c, index) {
                        ...
                        return InkWell(
                          onTap: () {
                            goTo(
                              RealPage(
                                proyecto: valoresToShow[index].proyecto,
